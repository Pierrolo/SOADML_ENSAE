...
def build_network_reg(inputs):
	"""
	Build network for the linear regression
	"""
	net = slim.fully_connected(inputs, 1, scope='fully_connected1', activation_fn=None)
	return net

def reg_loss(logits, labels):
	"""
	Calculates the loss from the preds and the labels.
	Args:
	logits: [batch_size].
	labels: [batch_size].
	Returns:
	loss: Loss tensor 
	""" 
	mse = tf.losses.mean_squared_error(labels=labels, predictions=logits, scope='mean_sq_err')
	return mse
...
def evaluation_reg(logits, labels):
    """Evaluate the quality of the logits at predicting the label.
	Args:
	logits: [batch_size, NUM_CLASSES].
	labels: [batch_size] 
